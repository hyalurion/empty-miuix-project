package com.hyalurion.exampleproject.data
 ...
 *   MainActivity / ExampleApp on the next launch. Therefore KILL only needs to clear the
 *   in-memory cache.
 *
...
class FairMemoryReceiver private constructor() : IBinder.DeathRecipient {
    ...
    private fun handleReceived(
        ...
        val replyData = Bundle().apply {
            putString("reply", "Example app handled $action for ${notifyTypeName(notifyType)}")
        }
     ...
     * and ExampleApp automatically restore the last tab/theme/language, so here we only need to
