package com.hyalurion.exampleproject.ui
...
import com.hyalurion.exampleproject.data.ThemeManager
import com.hyalurion.exampleproject.data.TabManager
import com.hyalurion.exampleproject.data.UpdateChecker
import com.hyalurion.exampleproject.ui.components.FloatingBottomBar
import com.hyalurion.exampleproject.ui.components.FloatingBottomBarItem
import com.hyalurion.exampleproject.ui.components.FloatingBottomBarMode
import com.hyalurion.exampleproject.data.AppDownloadManager
import top.yukonga.miuix.kmp.blur.rememberLayerBackdrop
import top.yukonga.miuix.kmp.blur.layerBackdrop
import com.hyalurion.exampleproject.ui.components.UpdateDialog
import com.hyalurion.exampleproject.ui.navigation.LicensesKey
import com.hyalurion.exampleproject.ui.navigation.SettingsKey
import com.hyalurion.exampleproject.ui.navigation.TAB_KEYS
import com.hyalurion.exampleproject.ui.navigation.TabNavKey
import com.hyalurion.exampleproject.ui.navigation.rememberExampleNavigator
import com.hyalurion.exampleproject.ui.navigation.tabKeyOf
import com.hyalurion.exampleproject.R
import com.hyalurion.exampleproject.ui.screens.LicensesScreen
import com.hyalurion.exampleproject.ui.screens.SettingsScreen
import top.yukonga.miuix.kmp.theme.ColorSchemeMode
import top.yukonga.miuix.kmp.theme.ThemeController
import com.hyalurion.exampleproject.ui.theme.ExampleTheme
import com.hyalurion.exampleproject.ui.theme.LanguageController
...
fun ExampleApp() {
    ...
    // Initial back stack: the saved tab is restored as the root. These keys are only applied the
    // first time the back stack is built; after a configuration change or process death the
    // persisted stack is restored instead.
    val initialBackStack = remember(savedTab) {
        arrayOf<NavKey>(tabKeyOf(savedTab))
    }

    // Navigation 3 back stack, wrapped so that call sites keep reading like the old NavController
    val navigator = rememberExampleNavigator(*initialBackStack)
    ...
    val selectedTab: TabNavKey by remember {
        derivedStateOf { backStack.lastOrNull { it is TabNavKey } as? TabNavKey ?: SettingsKey }
    }
    ...
    // Switching tabs pushes the tab onto the shared back stack.
    // launchSingleTop keeps a tap on the already selected tab from stacking a duplicate entry.
    ...
    // Move ExampleTheme call here to ensure theme changes are applied immediately
    val themeController = remember(currentThemeMode) {
        ThemeController(currentThemeMode)
    }
    ExampleTheme(controller = themeController, languageController = languageController) {
                            ...
                            if (!wide) {
                                Box(
                                    ...
                                    FloatingBottomBar(
                                        ...
                                        onSelected = { index ->
                                            onTabSelected(TAB_KEYS.getOrElse(index) { SettingsKey })
                                        },
                                        backdrop = backdrop,
                                        tabsCount = 1,
                                        mode = navMode,
