 ...
 */
@Stable
class ExampleNavigator(val backStack: NavBackStack<NavKey>) {
...
/**
 * Creates a [ExampleNavigator] whose back stack survives configuration changes and process death.
 *
 * [rememberNavBackStack] serializes the keys through `kotlinx.serialization`, which is why every
 * [ExampleNavKey] is annotated with `@Serializable`. [initial] is only used the very first time the
 * back stack is created; on restore the saved stack wins.
 */
@Composable
fun rememberExampleNavigator(vararg initial: NavKey): ExampleNavigator {
    val backStack = rememberNavBackStack(*initial)
    return remember(backStack) { ExampleNavigator(backStack) }
}
